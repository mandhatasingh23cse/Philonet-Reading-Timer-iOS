...
@Observable @MainActor final class ReaderViewModel {
    ...
    init(article: Article, repository: ArticleRepository, debug: DebugService, timer: TimerService? = nil, merger: MergeServiceProtocol = MergeService()) { self.article = article; self.repository = repository; self.debug = debug; self.timer = timer ?? TimerService(); self.merger = merger; persistedBase = article.totalReadingTime }
